...
(function() {
  ...
  var $this = this
    ...
    execute= function( complete ) {
      ...
      // before code is evaled
      if ( window.__before_code_execute )
        window.__before_code_execute();
      ...
      var code = [ $testing_framework ].concat( $tests ).join(';;')
        ...
        , execute = function() {
            ...
            // runs before testing
            if ( window.__before_test_execute )
              window.__before_test_execute();
      ...
      get('input', {
        load: function( data ) {
          ...
          for (var k in data.zones) {
            ...
            // scripts
            if ( /^(javascript|js)$/i.test( item.syntax ) )
              $eval( item.content );

            // styles
            else if ( /^css$/i.test( item.syntax ) )
              append_styles( item.content );

            // otherwise, look for markup
            else if ( target )
...
//       // update the ID if require
//       if ( zone.remove_id ) delete target.id;

//       // populate the area or run the script
//       var content = zone.content || '';
//       if ( /^script$/i.test( target.tagName ) ) $eval( content );
//       else if ( /^style$/i.test( target.tagName ) ) append_styles( content );
